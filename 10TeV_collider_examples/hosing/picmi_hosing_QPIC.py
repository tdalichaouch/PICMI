...
def run_qpad_sim1(z, gas_density):
	...
	kp = sim1.kp
	wp = sim1.wp
	sim1.init_grid(nx = nx, ny= ny, nz = nz, xmin = xmin/kp, xmax = xmax/kp, ymin = ymin/kp,\
	 ymax = ymax/kp, zmin = zmin/kp, zmax = zmax/kp)
	...
	sim1.add_gaussian_electron_bunch(beam2_charge, beam2_sigmas, n_macroparticles = beam2_macroparticles,  \
		bunch_rms_velocity = rms_vel, bunch_centroid_velocity = [0,0,gamma],\
		 bunch_centroid_position = beam2_centroid_position, alpha = alpha)
	...
	sim1.run_simulation(dt = dt_qpic/wp, tmax = (zsim1 + 0.01)/cst.c, nodes = QPIC_nodes,\
	 sim_dir = sim_dir, path_to_qpic = path_to_qpic)
